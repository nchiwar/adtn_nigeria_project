...
def home_view(request):
    # Final fix to ensure news items display
    news_items = News.objects.all().order_by('-date')
    print(f"Home View Debug - Count: {news_items.count()}, Query: {News.objects.all().query}, Items: {[str(n) for n in news_items]}")
    if not news_items.exists():
        print("Critical: No news items found. Check database or admin data.")
